...
%clear all; close all; load plot_gaussian_ellipsoid;
...
switch numel(m)
   case 2, [h,bp]=show2d(m(:),C,sdwidth,npts,axh);
   case 3, [h,bp]=show3d(m(:),C,sdwidth,npts,axh);
   otherwise
...
%-----------------------------
function [h,bp] = show2d(means, C, sdwidth, npts, axh)
...
h = plot(bp(1,:), bp(2,:), '-', 'parent', axh,'color',[0 0 0]);

%-----------------------------
function [h,bp] = show3d(means, C, sdwidth, npts, axh)
...
%h = surf(axh, xp,yp,zp);
%h = mesh(axh, xp,yp,zp);
h = mesh(axh, xp,yp,zp,'EdgeColor','black');  alpha(.1); %alpha is to make it transparent
%colormap(gray);