                                            ...
                                            const std::string & fixed_frame) const
{
  ...
  try
  {
    ...
    if(tf_exists)
    {
      geometry_msgs::PoseStamped person_pose_base_link;

      //transformPose(target_frame, target_time, pose_in, fixed_frame, pose_out);
      tf_listener_.transformPose(target_frame, request_time, pose, fixed_frame, person_pose_base_link);

      ROS_INFO("PeopleFollowerAlgorithm::transformGoal: frame_id=%s\tpose=(%f, %f)",
                                         ...
                                         const float & threshold_dist) const
{
  ...
  ROS_INFO("PeopleFollowerAlgorithm::udpateGoal:: last(%f,%f) current(%f,%f) needs_update=%d (%f>%f)",
