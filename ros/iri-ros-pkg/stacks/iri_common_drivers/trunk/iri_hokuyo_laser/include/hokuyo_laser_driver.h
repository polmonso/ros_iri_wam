...
#ifndef _hokuyo_laser_driver_h_
#define _hokuyo_laser_driver_h_

#include "iri_base_driver/iri_base_driver.h"
#include "iri_hokuyo_laser/HokuyoLaserConfig.h"

//include hokuyo_laser_driver main library
#include "hokuyo.h"
 ...
 * by IriBaseDriver.
 *
 * The HokuyoLaserDriver class must implement all specific driver requirements to
 ...
 *
 * The HokuyoLaserConfig.cfg needs to be filled up with those parameters suitable
 ...
 *
 */
class HokuyoLaserDriver : public iri_base_driver::IriBaseDriver
{
  private:
    // private attributes and methods
    CHokuyo *laser;
    CMutex driver_access;
    int number_of_scans;
    std::string id;
    THokuyo_serial_config usb_cfg;
    THokuyo_ethernet_config eth_cfg;
    THokuyo_specs specs;
    std::string ip;
    std::string port;
    bool use_eth;
  ...
  public:
    ...
    *
    * Define a Config type with the HokuyoLaserConfig. All driver implementations
    * will then use the same variable type Config.
    */
    typedef iri_hokuyo_laser::HokuyoLaserConfig Config;
    ...
    */
    ...
    // here define all hokuyo_laser_driver interface methods to retrieve and set
    ...
    */
    ...
    std::string get_new_scan_available_event();

    void getScan(THokuyo_scan & scan);

    char setType(const int & type_num);

    THokuyo_scan_config translateConfig(Config & cfg);
};
