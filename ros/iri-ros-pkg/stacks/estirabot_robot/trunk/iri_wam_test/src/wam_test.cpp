...
WamTest::WamTest() {
  ...
  port_name = ros::names::append(ros::this_node::getName(), "pose_matrix"); 
  this->tf_publisher = this->nh_.advertise<tf::tfMessage>(port_name, 5);
  
  // [init subscribers]
  port_name = ros::names::append(ros::this_node::getName(), "pose"); 
  this->pose_subscriber = this->nh_.subscribe(port_name, 5, &WamTest::pose_callback, this);
  port_name = ros::names::append(ros::this_node::getName(), "joint_states"); 
  this->joint_states_subscriber = this->nh_.subscribe(port_name, 5, &WamTest::joint_states_callback, this);
...
void WamTest::pose_callback(const geometry_msgs::PoseStamped::ConstPtr& msg) 
{ 
  //std::cout << msg->data << std::endl; 

}
void WamTest::joint_states_callback(const sensor_msgs::JointState::ConstPtr& msg) 
{ 
...
void WamTest::calculate_error(std::string target) {
